...
std::error_code MESYTEC_MCPD_EXPORT send_command(int sock, const CommandPacket &request);
std::error_code MESYTEC_MCPD_EXPORT receive_response(int sock, CommandPacket &response);

std::error_code MESYTEC_MCPD_EXPORT command_transaction(
    int sock,
    const CommandPacket &request,
    CommandPacket &response);

std::error_code MESYTEC_MCPD_EXPORT prepare_command_packet(
    CommandPacket &dest,
...
CommandPacket MESYTEC_MCPD_EXPORT make_command_packet(
...
std::error_code MESYTEC_MCPD_EXPORT mcpd_set_run_id(int sock, u8 mcpdId, u16 runId);

std::error_code MESYTEC_MCPD_EXPORT mcpd_reset_daq(int sock, u8 mcpdId);
std::error_code MESYTEC_MCPD_EXPORT mcpd_start_daq(int sock, u8 mcpdId);
std::error_code MESYTEC_MCPD_EXPORT mcpd_stop_daq(int sock, u8 mcpdId);
std::error_code MESYTEC_MCPD_EXPORT mcpd_continue_daq(int sock, u8 mcpdId);

std::error_code MESYTEC_MCPD_EXPORT mcpd_get_all_parameters(int sock, u8 mcpdId, McpdParams &dest);

std::error_code MESYTEC_MCPD_EXPORT mcpd_get_bus_capabilities(int sock, u8 mcpdId, BusCapabilities &caps);
std::error_code MESYTEC_MCPD_EXPORT mcpd_set_bus_capabilities(int sock, u8 mcpdId, u8 capBits, u8 &resultBits);

std::error_code MESYTEC_MCPD_EXPORT mcpd_set_timing_options(int sock, u8 mcpdId, TimingRole role, BusTermination term, bool extSync = false);

std::error_code MESYTEC_MCPD_EXPORT mcpd_set_master_clock_value(int sock, u8 mcpdId, u64 clock);

std::error_code MESYTEC_MCPD_EXPORT mcpd_setup_cell(
    int sock, u8 mcpdId,
    const CellName &cell,
    const TriggerSource &trigSource,
    u16 compareRegisterBitValue);

std::error_code MESYTEC_MCPD_EXPORT mcpd_setup_auxtimer(
    int sock, u8 mcpdId,
    u16 timerId,
    u16 compareRegisterValue);

std::error_code MESYTEC_MCPD_EXPORT mcpd_set_param_source(
    int sock, u8 mcpdId,
    u16 param,
    const DataSource &source);

std::error_code MESYTEC_MCPD_EXPORT mcpd_set_dac_output_values(
    int sock, u8 mcpdId,
...
std::error_code MESYTEC_MCPD_EXPORT mcpd_scan_busses(
    int sock, u8 mcpdId, std::array<u16, McpdBusCount> &dest);

std::error_code MESYTEC_MCPD_EXPORT mcpd_write_register(
    int sock, u8 mcpdId, u16 address, u32 value);

std::error_code MESYTEC_MCPD_EXPORT mcpd_read_register(
    int sock, u8 mcpdId, u16 address, u32 &dest);

std::error_code MESYTEC_MCPD_EXPORT read_peripheral_register(
    int sock, u8 mcpdId, u8 mpsdId,
    u16 registerNumber, u16 &dest);

std::error_code MESYTEC_MCPD_EXPORT write_peripheral_register(
    int sock, u8 mcpdId, u8 mpsdId,
...
std::error_code MESYTEC_MCPD_EXPORT mpsd_set_gain(
    int sock, u8 mcpdId,
    u8 mpsdId, u8 channel, u8 gain);

std::error_code MESYTEC_MCPD_EXPORT mpsd_set_threshold(
    int sock, u8 mcpdId,
    u8 mpsdId, u8 threshold);

std::error_code MESYTEC_MCPD_EXPORT mpsd_set_pulser(
    int sock, u8 mcpdId,
...
std::error_code MESYTEC_MCPD_EXPORT mpsd_set_mode(
    int sock, u8 mcpdId,
    u8 mpsdId,
    const MpsdMode &mode);

std::error_code MESYTEC_MCPD_EXPORT mpsd_set_tx_format(
    int sock, u8 mcpdId,
    u8 mpsdId,
    u16 txFormat);

std::error_code MESYTEC_MCPD_EXPORT mpsd_get_params(
    int sock, u8 mcpdId,
    u8 mpsdId,
    MpsdParameters &dest);
...
// MSTD specific
//
std::error_code MESYTEC_MCPD_EXPORT mstd_set_gain(
    int sock, u8 mcpdId,
