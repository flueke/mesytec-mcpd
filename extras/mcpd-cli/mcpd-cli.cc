...
void report_counters(const CountersReportInfo &info, const std::string &title = "readout")
{
    ...
    if (info.flags & CountersReportInfo::ReportDeltas)
    {
        spdlog::info("{}: deltas: packets={} (mcpd={}, mdll={}), bytes={}, timeouts={}, events={}",
                     title, deltas.packets, deltas.packetsByType[1], deltas.packetsByType[2],
    ...
    if (dt_s > 0)
    {
        spdlog::info("{}: rates: dt_s={}, packets/s={:.2f} (mcpd={}, mdll={}), MiB/s={:.2f}, "
                     "events/s={:.0f}",
                     title, dt_s, deltas.packets / dt_s, deltas.packetsByType[1] / dt_s,
                     deltas.packetsByType[2] / dt_s, deltas.bytes / dt_s / (1u << 20),
                     deltas.events / dt_s);
    }
}
...
struct ReplayCommand: public BaseCommand
{
    ...
    int runCommand(CliContext &ctx) override
    {
        ...
        // final report. lots of copy pasta :(
        {
            ...
            info.dt = std::chrono::duration_cast<std::chrono::microseconds>(elapsed);
            info.flags = CountersReportInfo::ReportRates | CountersReportInfo::ReportValues;
            report_counters(info, "replay final");
            fmt::print("\n");
            tReport = now;
            prevCounters = counters;
        }
